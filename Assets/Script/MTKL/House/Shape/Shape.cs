...
public class Shape : MonoBehaviour, IPointerDownHandler, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public void CreateShape(ShapeData shapeData)
    {
        ...
        for (var row = 0; row < shapeData.rows; row++)
        {
            for (var column = 0; column < shapeData.columns; column++)
            {
                ...
                {
                    _currentShape[currentIndextInList].SetActive(true);
                    _currentShape[currentIndextInList].GetComponent<RectTransform>().localPosition =
    ...
    private float GetYPositionForShapeSquare(ShapeData shapeData, int row, Vector2 moveDistance)
    {
        ...
        if (shapeData.rows > 1)
        {
            if (shapeData.rows % 2 != 0)
            {
                ...
                else if (row > middleSquareIndex) //往下走
                {
                    shiftOnY = moveDistance.y * -1;
                    shiftOnY *= multiplier;
                }
    ...
    private float GetXPositionForShapeSquare(ShapeData shapeData, int column, Vector2 moveDistance)
    {
        ...
        if (shapeData.columns > 1)
        {
            if (shapeData.columns % 2 != 0)
            {
                ...
                if (column < middleSquareIndex) //反方向
                {
